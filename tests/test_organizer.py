...
from omtriage.organizer import create_session_structure, group_files, organize_sessions
...
def test_organize_sessions_custom_gap(tmp_path):
    ...
    for i in range(3):
        path = tmp_path / f"test{i}.ORF"
        path.write_bytes(b"x" * 1024)  # Create dummy file
